...
using System.Timers;
using Atk_wsCatFuntLocation.Properties;
using BusinessLogic;
using Entidades;
using RepositorySql;
using ToolsTpm;
...
{
   public partial class UpdFunctLocation : ServiceBase
   {
      ...
      public UpdFunctLocation()
      {
         InitializeComponent();

         pathLog = @pathLog + "LogCatFL.txt";
      ...
      private void EjecutaProceso()
      {
         ...
         try
         {
            ...
            tw21.WriteLine(" Atk_wsCatFuntLocation - EjecutaProceso -Inicio de la actualizacion de datos, ==>  " + ",  " + DateTime.Now.ToString());
            tw21.Close();
            ...
            lstFl = datosSap.DatosFunctLoc(cnxSap, cnxSqlMT);

            SqlRepository repoSql = new SqlRepository();

            int resul = repoSql.GuardarCatFLSql(cnxSqlMT, lstFl, "CatFuntLocation", pathLog);
            ...
            tw2.WriteLine("Atk_wsCatFuntLocation - EjecutaProceso - Terminacion del proceso ==>  " + DateTime.Now.ToString());
            tw2.WriteLine("------------------------------------------------------------------------");
            tw2.Close();
         ...
         {
            string cMsj = "Atk_wsCatFuntLocation - EjecutaProceso - Error en la actualizacion ==>  " + ex.Message + ", fuente: " + ex.Source + ", fecha: " + DateTime.Now.ToString();
      ...
      {
         string cMsj = " Atk_wsCatFuntLocation - INICIO del servicio ==>  " + DateTime.Now.ToString();
      ...
      {
         string cMsj = "Atk_wsCatFuntLocation - PARO de servicio ==>  " + DateTime.Now.ToString();
         TextWriter tw = new StreamWriter(pathLog, true);
         tw.WriteLine("------------------------------------------------------------------------");
         tw.WriteLine(cMsj);
         tw.Close();
