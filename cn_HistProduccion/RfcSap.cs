...
namespace cn_HistProduccion
{
    public class RfcSap
    {
        ...
        public void LeeMatNotificado(RfcDestination destination, SqlConnection cnx, DateTime pFI, DateTime pFF)
        {
            try
            {
                ...
                using (SqlBulkCopy bulkCopy = new SqlBulkCopy(cnx))
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        TextWriter twe = new StreamWriter(cn_HistProduccion.Program.pathe, true);
                        twe.WriteLine("cn_HistProduccion FALLA de RFC: Al agregar Notificado al SQL:   --> fecha de " + DateTime.Now.ToString());
                        twe.Close();
                        EnviarCorreo email = new EnviarCorreo();
                        email.Enviar("cn_HistProduccion FALLA de RFC: Al agregar Notificado al SQL:   --> fecha de " + DateTime.Now.ToString());
                    }
                }
                #endregion
            ...
            {
                TextWriter twe = new StreamWriter(cn_HistProduccion.Program.pathe, true);
                twe.WriteLine("cn_HistProduccion FALLA en RFC - LeeMatNotificado: " + ex.Message + " --> fecha de " + DateTime.Now.ToString());
                twe.Close();
                EnviarCorreo email = new EnviarCorreo();
                email.Enviar("cn_HistProduccion FALLA en RFC: " + ex.Message + " --> fecha de " + DateTime.Now.ToString());
            }
            catch (RfcLogonException ex)
            {
                // user could not logon...
                TextWriter twe = new StreamWriter(cn_HistProduccion.Program.pathe, true);
                twe.WriteLine("cn_HistProduccion FALLA en Login del RFC  - LeeMatNotificado: " + ex.Message + " --> fecha de " + DateTime.Now.ToString());
                twe.Close();
                EnviarCorreo email = new EnviarCorreo();
                email.Enviar("cn_HistProduccion FALLA en Login del RFC  - LeeMatNotificado: " + ex.Message + " --> fecha de " + DateTime.Now.ToString());
            }
            catch (RfcAbapRuntimeException ex)
            {
                // serious problem on ABAP system side...
                TextWriter twe = new StreamWriter(cn_HistProduccion.Program.pathe, true);
                twe.WriteLine("cn_HistProduccion FALLA en RFC  - LeeMatNotificado: serious problem on ABAP system side, " + ex.Message + " --> fecha de " + DateTime.Now.ToString());
                twe.Close();
                EnviarCorreo email = new EnviarCorreo();
                email.Enviar("cn_HistProduccion.Program. FALLA en RFC  - LeeMatNotificado: serious problem on ABAP system side, " + ex.Message + " --> fecha de " + DateTime.Now.ToString());
            }
            catch (RfcAbapBaseException ex)
            {
                ...
                TextWriter twe = new StreamWriter(cn_HistProduccion.Program.pathe, true);
                twe.WriteLine("cn_HistProduccion.Program. FALLA en RFC - LeeMatNotificado: , " + ex.Message + " --> fecha de " + DateTime.Now.ToString());
                twe.Close();
                EnviarCorreo email = new EnviarCorreo();
                email.Enviar("cn_HistProduccion.Program. FALLA en RFC - LeeMatNotificado: , " + ex.Message + " --> fecha de " + DateTime.Now.ToString());
            }
