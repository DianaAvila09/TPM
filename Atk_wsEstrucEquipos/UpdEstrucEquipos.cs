using Atk_wsEstrucEquipos.Properties;
using BusinessLogic;
using Entidades;
using RepositorySql;
using System;
...
{
   public partial class UpdEstrucEquipos : ServiceBase
   {
      ...
      public UpdEstrucEquipos()
      {
         InitializeComponent();

         pathLog = @pathLog + "LogCatEstrucEquipos.txt";
      ...
      private void EjecutaProceso()
      {
         ...
         try
         {
            ...
            tw21.WriteLine(" Atk_wsEstrucEquipos - EjecutaProceso -Inicio de la actualizacion de datos, ==>  " + ",  " + DateTime.Now.ToString());
            tw21.Close();
            ...
            lstEE = datosSap.DatosStructEquipos(cnxSap, cnxSqlMT);
            SqlRepository repoSql = new SqlRepository();
            int resul = repoSql.GuardarCatEESql(cnxSqlMT, lstEE, "CatEstrucEquipos", pathLog);
            ...
            tw2.WriteLine("Atk_wsEstrucEquipos - EjecutaProceso - Terminacion del proceso ==>  " + DateTime.Now.ToString());
            tw2.WriteLine("------------------------------------------------------------------------");
            tw2.Close();
         ...
         {
            string cMsj = "Atk_wsEstrucEquipos - EjecutaProceso - Error en la actualizacion ==>  " + ex.Message + ", fuente: " + ex.Source + ", fecha: " + DateTime.Now.ToString();
            TextWriter tw23 = new StreamWriter(pathLog, true);
            tw23.WriteLine(cMsj);
            tw23.Close();
            tool.GuardarError(cnxSqlMT, ex.Message, ex.StackTrace, "EjecutaProceso", "UpdEstrucEquipos");
         }
      ...
      {
         string cMsj = " Atk_wsEstrucEquipos - INICIO del servicio ==>  " + DateTime.Now.ToString();
      ...
      {
         string cMsj = "Atk_wsEstrucEquipos - PARO de servicio ==>  " + DateTime.Now.ToString();
         TextWriter tw = new StreamWriter(pathLog, true);
         tw.WriteLine("------------------------------------------------------------------------");
         tw.WriteLine(cMsj);
         tw.Close();
